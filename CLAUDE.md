...
- **Domain model** (`sip/`): `Package`, `Entity`, `Representation`, `File`. All identifiers take the form `uuid-<uuid>`, carried as plain strings and checked by `sip.ValidateIdentifier`. `File.Path` is the href relative to the METS document that references the file (documented on the field — subtle and load-bearing); `File.Source` records where essence comes from; `Entity.Description` carries the decoded descriptive metadata (`metadata.Terms`) until the writer serializes it. `sip.Event` is an empty stub.
