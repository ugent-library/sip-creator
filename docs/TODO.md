...
- **Characterization as sidecar input; METS `@MIMETYPE` fix** → [characterization-sidecar plan](plans/characterization-sidecar.md) + its ADR-0009 (Step A2). Kills the in-process `sf` exec path in favor of consuming a pre-computed `sf -hash md5 -json` report as optional input (ADR-0008 symmetry: data, not an interface), then fixes the hardcoded `MIMETYPE="text/xml"` lie from the sidecar's mime with an `application/octet-stream` fallback. Owns and retires two former open questions: *batch format identification per representation* (dissolved — one sf run per package by construction; the 2026-07-16 exec-vs-library weighing is preserved in the plan/ADR) and *METS `@MIMETYPE` is a hardcoded lie* (source of truth decided: characterizer's assertion or admitted ignorance, never a guess).
