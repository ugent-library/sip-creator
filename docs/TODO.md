...
- **New input contract (convention-based folders + key–value `metadata.csv`)** → owned by the [input-convention plan](plans/input-convention.md) with [ADR-0010](decisions/0010-config-over-self-describing-input.md) (the config-over-self-describing-input trade-off); supersedes the earlier loose "CSV descriptive-metadata input" idea. I1 (the `cli/input` package) landed 2026-08-19. The plan also closes the `mets/@TYPE` and representation-naming items below and the §8-deferred features stay chosen against (explicit manifest, operator-supplied descriptive XML, multiple intellectual entities, BagIt input, per-package overrides).
- **`mets/@TYPE` should be operator- or content-selectable per package** → the registry value (`Photographs – Digital`) is *legal* meemoo-1.2 vocabulary and apt for the sample fixture, but a real archive packages more than photographs; the remaining work is input/config plumbing for the content category, not a vocabulary fix; it is scheduled as [input-convention plan](plans/input-convention.md) I4. Ref: [CSIP2](https://earkcsip.dilcis.eu/#CSIP2), meemoo 1.2 §package METS.
- **Representation directory naming** → decided by [input-spec.md](input-spec.md) §2 and the [input-convention plan](plans/input-convention.md): operator folder names are free-form labels in the portable character set (`A–Za–z0–9._-`, satisfying CSIP's uniqueness and staying href/OBJID-safe); the tool decides package-side directory naming per profile, so the `representation_([0-9]+)$` regex and its silent skip disappear at the I3 cut-over. The meemoo-2.x "OBJID must equal dir name" coupling lands with the 2.x migration item below.
...
- **Library builder API: embeddability requirements.** So larger ingest-automation systems can drive the library without touching the CLI's input convention, the builder API must: accept essence as streams (`io.Reader`/`fs.FS` + logical path), not only filesystem paths; accept pre-computed fixity and only compute checksums when none are supplied; take descriptive metadata as data (structs), not files to parse; take administrative metadata (agents, submission agreement, record status, content category) as data, for which `sip.Spec`/`sip.Agent` (landed with [refactoring plan](archive/refactoring-plan.md) Steps 7–8) is the vehicle; accept a caller-supplied package identifier (updates reuse the original `mets/@OBJID`) and mint a UUID only when none is given; keep representation labels free-form in the model (the profile decides SIP-side directory naming); build to a destination the caller controls. See the [CLI/library boundary](sip-creator-design.md#clilibrary-boundary) in the design doc.
  - **It must also ship `sip.Package.Validate()`**, the first domain-validation method (`Validate` methods on `sip/` types per `CLAUDE.md`), called between assemble and write, covering at least: identifier uniqueness across the graph (a set-check; decided 2026-07 when Step 7 removed the inert METS `idStore`: no minting-time machinery, catch *systematic* duplication like a node wired in twice, the ~10⁻²⁵ UUIDv4 collision comes free, commons-ip's `xs:ID` XSD check stays the document-level net) and the no-empty-`Mime` invariant ([ADR-0009](decisions/0009-characterization-as-sidecar-input.md)). **Deliberately deferred to this work** (2026-08-18): with the CLI as the graph's only producer, assemble's tests and the construction sites already guarantee both invariants, and a runtime re-check would be anticipatory; the invariants become real input-error classes only when callers construct graphs and supply identifiers. Revisit when the refactoring arc is finished and this API work starts. **Update 2026-08-19:** that trigger fires early for the *input-data* slice: the [input-convention plan](plans/input-convention.md)'s I3 cut-over makes `profiles.Config` a constructible input, so builder-side validation of caller-supplied data (representations non-empty, descriptive identity, term validity, labels, path uniqueness) moved into that plan (I2/I3, see its "Validation splits in two"). What remains here: the graph-level checks (identifier uniqueness across the graph and no-empty-`Mime`).
