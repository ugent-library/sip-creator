...
- A known cost of hand-authored templates: literals meant to be profile-specific (e.g. the meemoo `TYPE` and profile URL) currently sit inside shared templates. That is a factoring problem to fix by parameterizing the templates, not a reason to abandon them — see the [refactoring plan](../archive/refactoring-plan.md).
