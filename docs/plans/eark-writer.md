...
- **RODA's ingest** never runs that validator: `EARKSIP2ToAIPPlugin` calls
  `new EARKSIP().parse(...)` and accepts or rejects on the *parse-level* report (METS readable,
...
- **Documentation**: the input tree gains an optional package-level
  ...
  (activating CSIP94 when present). Whether the validator *also* requires
...
- **E2: the family mechanism.** `Family` type + constants + field; `basic` declares
