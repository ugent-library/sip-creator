...
- **`encoders/`** — one package per metadata standard (`metadata`, `mets`, `premis`), each a thin `Encode*(io.Writer, …)` API backed by a `text/template`. **No XML library** — all XML is generated from templates (see [ADR-0002](decisions/0002-xml-via-text-template.md)). METS ID minting lives in the mets encoder's `identifier()`.
...
The [refactoring plan](plans/refactoring-plan.md) and its companions have shipped: the assemble/emit split, `store/` package, and declarative `Definition` registry (Phases 0–2), the optional-enrichment decision ([format-identification plan](plans/format-identification-optional.md), since superseded in part), and the eark profile ([eark-writer plan](plans/eark-writer.md), awaiting field acceptance on RODA). Mid-flight now: the [characterization-sidecar plan](plans/characterization-sidecar.md) — Step A (sidecar input, this section's current description) has landed; Step B (the METS `@MIMETYPE` fix) remains.
