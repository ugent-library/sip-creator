...
- **Representation** (`sip.Representation`): a typed grouping of the essence (e.g. a master vs. a derivative). Holds its `Name` (the package-side directory name under `representations/`, taken verbatim from the producer's label), its `Label` (the producer's name, emitted as the representation METS `mets/@LABEL`), `Identifier`, its `Files`, its own representation-level `MetsFile` and `PremisFile`, its own METS `Declaration` (set by the assembler: the profile declaration, or, in the eark profile, a copy that types the representation by its label in both the `TYPE` and `CONTENTINFORMATIONTYPE` attribute pairs so ingest systems show the label as the representation's type; [ADR-0013](decisions/0013-representation-type-from-label.md)), and a back-reference to its `Entity`.
