...
- **Representation** (`sip.Representation`): a typed grouping of the essence (e.g. a master vs. a derivative). Holds its `Name` (the package-side directory name under `representations/`, taken verbatim from the producer's name), its `Label` (the display name, emitted as the representation METS `mets/@LABEL`), `Identifier`, its `Files`, its own representation-level `MetsFile` and `PremisFile`, its own METS `Declaration` (set by the assembler: the profile declaration, or, in the eark profile, a copy that types the representation by its resolved type in both the `TYPE` and `CONTENTINFORMATIONTYPE` attribute pairs so ingest systems show it as the representation's type; [ADR-0013](decisions/0013-representation-type-from-label.md)), and a back-reference to its `Entity`. The producer supplies name, label, and type on `profiles.SourceRepresentation`; label and type are optional and resolve along the name → label → type cascade in the assembler.
...
uuid-<uuid>/
  ...
  representations/
    <name>/                             one folder per representation, named by the producer's label
