package profiles

import (
	"bytes"
	"crypto/md5"
	"encoding/hex"
	"fmt"
	"io"
	"log/slog"
	"os"
	"path"
	"path/filepath"
	"regexp"
	"strconv"
	"time"

	"github.com/ugent-library/sip-creator/encoders/metadata"
	"github.com/ugent-library/sip-creator/encoders/mets"
	"github.com/ugent-library/sip-creator/encoders/premis"
	"github.com/ugent-library/sip-creator/formats"
	"github.com/ugent-library/sip-creator/schemas"
	"github.com/ugent-library/sip-creator/sip"
)

type Config struct {
	Source      string
	Destination string
	Logger      *slog.Logger
	Formats     formats.Identificator
}

type Profile struct {
	BaseDir string
	InDir   string
	Logger  *slog.Logger
	Formats formats.Identificator
}

func New(config *Config) *Profile {
	return &Profile{
		BaseDir: config.Destination,
		InDir:   config.Source,
		Logger:  config.Logger,
		Formats: config.Formats,
	}
}

func (p *Profile) createPackage() *sip.Package {
	// Create skeleton
	pkg := sip.NewPackage(p.BaseDir)

	// @TODO Better path handling
	p.BaseDir = fmt.Sprintf("%s/%s", p.BaseDir, pkg.Identifier)

	packageDirs := []string{
		fmt.Sprintf("%s/metadata/descriptive", pkg.Location),
		fmt.Sprintf("%s/metadata/preservation", pkg.Location),
		fmt.Sprintf("%s/representations", pkg.Location),
		fmt.Sprintf("%s/schemas", pkg.Location),
	}

	for _, pd := range packageDirs {
		createDir(pd)
	}

	// Create schema files
	files := p.createSchemaFiles(fmt.Sprintf("%s/schemas", pkg.Location))
	pkg.AddSchemaFiles(files)

	// Create a new package
	return pkg
}

func (p *Profile) createIntellectualEntity() *sip.Entity {
	return sip.NewEntity()
}

func (p *Profile) createSchemaFiles(dest string) []*sip.File {
	var files []*sip.File
	for name, val := range schemas.Get() {
		path := fmt.Sprintf("%s/%s", dest, name)

		file := createMetadataFile(path, func(w io.Writer) error {
			_, err := w.Write(val)
			return err
		})

		file.Path = path[len(p.BaseDir)+1:]

		files = append(files, file)
	}

	return files
}

type Description interface {
	GetLocalIdentifier(scheme string) string
	SetObjectIdentifier(id string)
}

func (p *Profile) createDescriptiveFile(src, dir string, fn func(d Description)) *sip.File {
	fi, err := os.Lstat(src)
	if err != nil {
		panic(err)
	}

	if fi.IsDir() {
		panic(fmt.Sprintf("%s is a directory, not a metadata file.", src))
	}

	base := path.Base(src)
	ext := path.Ext(base)
	name := base[0:len(base)-len(ext)] + ".xml"

	dest := fmt.Sprintf("%s/%s", dir, name)

	f, err := os.Open(src)
	if err != nil {
		panic(err)
	}
	defer f.Close()

	d := metadata.Decode(f)

	// Per the spec, we want to swap in the premis identifier for dcterms:identifier,
	//   and swap out existing dcterms:identifier values from the source, keeping them with
	//   the entity when we generate the premis file. Do this in a callback.
	fn(d)

	file := createMetadataFile(dest, func(w io.Writer) error {
		return metadata.Encode(w, d)
	})

	file.Path = dest[len(p.BaseDir)+1:]

	return file
}

func (p *Profile) createRepresentation(label string) *sip.Representation {
	createDir(fmt.Sprintf("%s/representations/%s/data", p.BaseDir, label))
	createDir(fmt.Sprintf("%s/representations/%s/metadata/preservation", p.BaseDir, label))

	r := sip.NewRepresentation(label)

	// TODO generate an optional description file (dc+schema, dc, mods) for representations
	//   e.g. when licensing overrides the licensing of the associated entity

	return r
}

func (p *Profile) eachDirectory(fn func(dir string, r *sip.Representation)) {
	err := filepath.Walk(p.InDir, func(dir string, info os.FileInfo, err error) error {
		if !info.IsDir() {
			return nil
		}

		// TODO fix case "representation_0"
		label := path.Base(dir)
		rx, _ := regexp.Compile("representation_([0-9]+)$")
		if !rx.MatchString(label) {
			return nil
		}

		r := p.createRepresentation(label)

		fn(dir, r)

		return nil
	})

	if err != nil {
		panic(err)
	}
}

func (p *Profile) eachEssenceFile(dir, label string, fn func(r *sip.File)) {
	err := filepath.Walk(dir, func(src string, info os.FileInfo, err error) error {
		if info.IsDir() {
			return nil
		}

		// TODO ignore all descriptive files per the spec (dc, dc+schema, mods)

		base := fmt.Sprintf("%s/representations/%s", p.BaseDir, label)
		dest := fmt.Sprintf("%s/data/%s", base, path.Base(src))
		copy(src, dest)

		// TODO make this a registrable identificator, add support for Droid as well
		// formatter := siegfried.New("sf", []string{"-hash", "md5", "-json"})
		// f := p.FileForm.Process(dest)
		f := p.Formats.Process(dest)

		f.Path = dest[len(base)+1:]

		fn(f)

		return nil
	})

	if err != nil {
		panic(err)
	}
}

func (p *Profile) generatePackagePremis(e *sip.Entity) *sip.File {
	// TODO also account for sub-iE(s) tied to the root entity
	path := fmt.Sprintf("%s/metadata/preservation/premis.xml", p.BaseDir)
	f := createMetadataFile(path, func(w io.Writer) error {
		return premis.EncodeEntity(w, e)
	})

	// Set the relative path within the package for METS
	f.Path = path[len(p.BaseDir)+1:]

	return f
}

func (p *Profile) generateRepresentationPremis(r *sip.Representation) *sip.File {
	// path := fmt.Sprintf("%s/representations/%s/metadata/preservation/premis.xml", p.BaseDir, r.Label)

	base := fmt.Sprintf("%s/representations/%s", p.BaseDir, r.Label)
	path := fmt.Sprintf("%s/metadata/preservation/premis.xml", base)

	f := createMetadataFile(path, func(w io.Writer) error {
		return premis.EncodeRepresentation(w, r)
	})

	f.Path = path[len(base)+1:]

	return f
}

func (p *Profile) generatePackageMets(pkg *sip.Package) *sip.File {
	path := fmt.Sprintf("%s/METS.xml", p.BaseDir)
	f := createMetadataFile(path, func(w io.Writer) error {
		return mets.EncodePackage(w, pkg)
	})

	f.Path = path[len(p.BaseDir)+1:]

	return f
}

func (p *Profile) generateRepresentationMets(r *sip.Representation) *sip.File {
	path := fmt.Sprintf("%s/representations/%s/METS.xml", p.BaseDir, r.Label)
	f := createMetadataFile(path, func(w io.Writer) error {
		return mets.EncodeRepresentation(w, r)
	})

	f.Path = path[len(p.BaseDir)+1:]

	return f
}

func createMetadataFile(dest string, fn func(w io.Writer) error) *sip.File {
	hash := md5.New()
	var buf1, buf2 bytes.Buffer
	w := io.MultiWriter(&buf1, &buf2)

	if err := fn(w); err != nil {
		panic(err)
	}

	if _, err := io.Copy(hash, &buf1); err != nil {
		panic(err)
	}

	of, err := os.OpenFile(dest, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0600)
	if err != nil {
		panic(err)
	}
	defer of.Close()

	buf := make([]byte, 2048)
	for {
		n, err := buf2.Read(buf)
		if err != nil && err != io.EOF {
			panic(err)
		}
		if n == 0 {
			break
		}

		if _, err := of.Write(buf[:n]); err != nil {
			panic(err)
		}
	}

	info, err := of.Stat()
	if err != nil {
		panic(err)
	}

	file := sip.NewFile()
	file.Name = path.Base(dest)
	file.Size = strconv.Itoa(int(info.Size()))
	file.Checksum = hex.EncodeToString(hash.Sum(nil))
	file.Created = info.ModTime().Format(time.RFC3339Nano)

	return file
}

func copy(src, dest string) {
	ipf, err := os.Open(src)
	if err != nil {
		panic(err)
	}

	of, err := os.OpenFile(dest, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0600)
	if err != nil {
		panic(err)
	}
	defer of.Close()

	buf := make([]byte, 2048)
	for {
		n, err := ipf.Read(buf)
		if err != nil && err != io.EOF {
			panic(err)
		}
		if n == 0 {
			break
		}

		if _, err := of.Write(buf[:n]); err != nil {
			panic(err)
		}
	}
}

func createDir(path string) {
	err := os.MkdirAll(path, 0775)
	if err != nil {
		panic(err)
	}
}
